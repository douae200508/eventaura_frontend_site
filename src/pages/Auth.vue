<template>
  <div class="min-h-screen bg-[#060d2e] flex items-center justify-center p-4">
    ...
    <!-- Card -->
    <div class="relative w-full max-w-5xl bg-white rounded-3xl shadow-2xl overflow-hidden flex" style="min-height: 620px;">

      <!-- ───── LEFT PANEL (form) ───── -->
      <div class="w-full lg:w-1/2 flex flex-col justify-center p-10 md:p-12 relative z-10 bg-white">

        <!-- Logo -->
        <div
          class="text-xl font-light tracking-widest uppercase mb-8 text-[#0a0f2e]"
        ...
        <transition name="slide-fade" mode="out-in">
          ...
          <!-- SIGN UP - VERSION AMÉLIORÉE AVEC 2 COLONNES -->
          <div v-else key="signup">
            <h1 class="text-3xl font-semibold text-gray-900 mb-1">Create account</h1>
            <p class="text-gray-400 text-sm mb-6">Join EventAura and start exploring</p>
            ...
            <form @submit.prevent="handleSignUp" class="space-y-4">
              ...
              <!-- LIGNE 2 : Email + Téléphone (2 colonnes) -->
              <div class="grid grid-cols-2 gap-4">
                <div class="relative">
                  <input
              ...
              <div class="grid grid-cols-2 gap-4">
                ...
                <div class="relative">
                  <!-- Remplacer la ligne Role par Entreprise -->
<!-- ❌ Supprimer le select role -->

<!-- ✅ Ajouter champ entreprise -->
<div class="relative">
    <input
        v-model="signUpForm.entreprise"
        type="text"
        placeholder=" "
        class="peer w-full px-0 pt-5 pb-2 border-0 border-b 
               border-gray-200 focus:border-blue-500 focus:ring-0 
               outline-none text-gray-900 text-sm bg-transparent 
               transition-colors"
    />
    <label class="absolute left-0 top-0 text-xs text-gray-400 
        ...
        Company (optional)
              ...
              <!-- LIGNE 4 : Password + Confirm Password (2 colonnes) -->
              <div class="grid grid-cols-2 gap-4">
                <div class="relative">
                  <input
                ...
                <div class="relative">
                  ...
                  <p v-if="passwordMismatch" class="absolute -bottom-5 left-0 text-red-400 text-[11px]">Passwords do not match</p>
                </div>
              </div>
              ...
              <button
                ...
                class="w-full bg-[#0a0f2e] text-white py-3.5 rounded-full text-sm font-medium hover:bg-blue-700 transition-all duration-300 disabled:opacity-40 disabled:cursor-not-allowed mt-2"
              >
      ...
      <!-- ───── RIGHT PANEL (decorative - espace bleu amélioré) ───── -->
      <div class="hidden lg:flex w-1/2 relative bg-gradient-to-br from-[#0a0f2e] to-[#1a2560] flex-col items-center justify-center p-14 overflow-hidden">
        ...
        <div class="relative z-10 w-full max-w-xs space-y-3 mb-10">
          <div v-for="card in decorCards" :key="card.label"
            class="flex items-center gap-3 bg-white/5 border border-white/10 rounded-2xl px-4 py-3 backdrop-blur-sm hover:bg-white/10 transition-all duration-300 cursor-pointer">
        ...
        <!-- Séparateur -->
        <div class="relative z-10 w-full max-w-xs my-4">
        ...
        <div class="relative z-10 mt-6 flex gap-4 text-center">
          <div>
            <p class="text-white text-lg font-semibold">500+</p>
            <p class="text-white/30 text-[10px] uppercase tracking-wide">Events</p>
          </div>
          <div class="w-px bg-white/20"></div>
          <div>
            <p class="text-white text-lg font-semibold">10k+</p>
            <p class="text-white/30 text-[10px] uppercase tracking-wide">Users</p>
          </div>
          <div class="w-px bg-white/20"></div>
          <div>
            <p class="text-white text-lg font-semibold">50+</p>
            <p class="text-white/30 text-[10px] uppercase tracking-wide">Cities</p>
          </div>
        </div>
      </div>
    </div>
...
const handleSignIn = async () => {
    ...
    try {
        ...
        const response = await axios.post(`${API}/login`, {
            ...
            mot_de_passe: signInForm.value.password // ✅ mot_de_passe
        })
...
const handleSignUp = async () => {
    ...
    try {
        ...
        const response = await axios.post(`${API}/register`, {
            ...
            entreprise:   signUpForm.value.entreprise  || null, // ✅ ajouté
            mot_de_passe: signUpForm.value.mot_de_passe
            // ❌ roles supprimé → géré par défaut ["client"]
            // ❌ mot_de_passe_confirmation supprimé
            // ❌ role supprimé
        })
...
const decorCards = [
    ...
    {
        label: 'UI/UX Workshop',
        sub:   'Apr 22 · Online',
        color: 'bg-purple-500/30',
        icon:  'M9.53 16.122a3 3 0 00-5.78 1.128'
    },
    {
